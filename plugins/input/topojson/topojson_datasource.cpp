...
struct collect_attributes_visitor : public boost::static_visitor<void>
{
    ...
    void operator() (GeomType const& g)
    {
        if (g.props)
        {
            BOOST_FOREACH (auto const& p , *g.props)
            {
                desc_.add_descriptor(mapnik::attribute_descriptor(boost::get<0>(p),
                    boost::apply_visitor(attr_value_converter(),boost::get<1>(p))));
            }
        }
    }
};
    ...
    tree_(16,1)
{
    ...
    std::size_t count = 0;
    BOOST_FOREACH (auto const& geom , topo_.geometries)
    {
...
mapnik::featureset_ptr topojson_datasource::features(mapnik::query const& q) const
{
    ...
    if (extent_.intersects(b))
    {
        ...
        return boost::make_shared<topojson_featureset>(topo_, *tr_, tree_.find(box));
    }
...
mapnik::featureset_ptr topojson_datasource::features_at_point(mapnik::coord2d const& pt, double tol) const
{
    ...
    BOOST_FOREACH (auto const& attr_info , desc_.get_descriptors())
    {
